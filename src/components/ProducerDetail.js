...
export default function ProducerDetail() {
  ...
  const images = (Array.isArray(gallery) && gallery.length ? gallery : logo ? [logo] : []).slice(0, 12);

  if (loading) {
    return <div className="max-w-md mx-auto px-3 pt-6 text-white/80">Загрузка</div>;
  }
  if (error) {
    return <div className="max-w-md mx-auto px-3 pt-6 text-red-400">Ошибка: {String(error.message || error)}</div>;
  }
  if (!producer) {
      ...
      <div className="max-w-md mx-auto px-3 pt-3 space-y-3">
          ...
          <div className="relative w-full rounded-xl overflow-hidden border border-white/10">
            ...
            <div className="absolute inset-x-0 bottom-0 p-3 bg-gradient-to-t from-black/60 via-black/10 to-transparent">
              ...
              <div className="mt-1 flex items-center gap-1.5 flex-wrap">
                ...
                {badges?.map((b, i) => (
                  <BadgeHint key={i} color={b === 'Честный знак' || b === 'Меркурий' ? 'blue' : 'gray'} label={b} hint={BADGE_INFO[b]} />
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="glass-card p-3">
          ...
          {(description || fullDescription) && (
            <div className="mt-2 text-white/90 text-[14px] leading-relaxed">{fullDescription || description}</div>
          )}
          ...
          <div className="mt-2 grid grid-cols-2 gap-2 text-white/90 text-[13px]">
            ...
            {!!exportMarkets?.length && <Fact icon="" title="Экспорт" value={exportMarkets.slice(0, 3).join(', ') + (exportMarkets.length > 3 ? '' : '')} />}
          </div>
        </div>
        ...
        {site && (
          <div className="glass-card p-3">
            ...
            <a href={site.startsWith('http') ? site : `https://${site}`} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 text-[rgba(59,175,218,0.95)] font-bold">
              Открыть сайт
            </a>
          </div>
        )}
...
function BadgeHint({ label, hint, color = 'gray' }) {
  ...
  return <span className={`${base} ${palette}`} title={hint || label}>{label}</span>;
}
