    ...
    <div className="bg-black min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-20 w-full bg-black/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center gap-3">
          ...
          <h2 className="ml-auto mr-auto text-white font-bold text-lg">{CATEGORY_LABELS[ad.category]}</h2>
          <span className="w-16" />
        </div>
      </div>
        ...
        <div className={`glass-card p-3 ${ad.premium ? "premium" : ""}`}>
          ...
          <div className="relative w-full aspect-square rounded-xl overflow-hidden border border-white/10 mb-3">
            <Swiper spaceBetween={12} slidesPerView={1} style={{ borderRadius: 12 }}>
              {(ad.images?.length ? ad.images : ["/images/no-image.webp"]).map((src, i) => (
                <SwiperSlide key={i}>
                  <img src={src} alt={`${ad.title} ${i+1}`} className="w-full h-full object-cover" onError={(e)=>{e.currentTarget.src="/images/no-image.webp"}} />
                </SwiperSlide>
              ))}
            </Swiper>
            ...
            {ad.verified && (
              <div className="absolute top-2 left-2">
                <span className="px-2 py-0.5 rounded-full text-[11px] font-bold text-white border border-white/20 bg-green-600/70 backdrop-blur-sm">✅ Проверенный</span>
              </div>
            )}
            {ad.premium && (
              <div className="absolute top-2 right-2 w-6 h-6 rounded-full border border-[rgba(59,175,218,.7)] bg-white/10 grid place-items-center backdrop-blur-sm" title="Premium">
          ...
          {ad.documents?.length > 0 && (
            <div className="flex flex-wrap gap-2 items-center text-xs text-blue-200 mb-3">
              {ad.documents.map((doc, i) => (<span key={i} className="bg-blue-900/40 px-3 py-1 rounded-full border border-white/10">📄 {doc}</span>))}
            </div>
          )}

          {ad.contact && (
            <a href={`tel:${ad.contact.replace(/[^+\d]/g, '')}`} className="mt-2 w-full flex items-center justify-center gap-2 bg-[rgba(59,175,218,.9)] hover:bg-[rgba(59,175,218,1)] text-white font-bold rounded-xl py-3 shadow-lg transition text-base">
