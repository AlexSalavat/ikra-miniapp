...
const exampleAds = [
  { id: 1, title: "Икра кеты солёная, 2024", images: ["/images/ikra1.webp"], category: "ikra", company: "ООО КамчатИкра", region: "Камчатка", price: "3 200 ₽/кг", verified: true, premium: true },
  { id: 2, title: "Горбуша свежемороженая, опт", images: ["/images/ryba1.webp"], category: "ryba", company: "ООО РыбСнаб", region: "Сахалин", price: "от 180 ₽/кг", verified: true },
  { id: 3, title: "Краб живой, Владивосток", images: ["/images/krab.webp"], category: "krab", company: "КрабСнаб", region: "Приморье", price: "12 000 ₽/шт" },
  { id: 4, title: "Мидии премиум", images: ["/images/mor.webp"], category: "mor", company: "Море Прим", region: "Владивосток", price: "850 ₽/кг" },
];
    ...
    <div className="bg-black min-h-screen pb-24">
      ...
      <div className="sticky top-0 z-20 w-full bg-black/80 backdrop-blur-md border-b border-white/10">
        <div className="max-w-md mx-auto px-3 py-3 flex items-center gap-3">
          ...
          <h2 className="ml-auto mr-auto text-white font-bold text-lg">{CATEGORY_LABELS[category] || "Объявления"}</h2>
          <span className="w-16" />
        </div>
      ...
      <div className="max-w-md mx-auto px-3 pt-3 grid grid-cols-2 gap-3">
        ...
        {filtered.length===0 && <div className="col-span-2 text-center text-white/70 mt-10">Нет объявлений</div>}
      </div>
    </div>
  );
}
    ...
    <Link to={`/market/sell/detail/${ad.id}`} className="block">
      <div className={`glass-card p-2 ${ad.premium ? "premium" : ""}`}>
        <div className="relative aspect-square rounded-lg overflow-hidden border border-white/10">
          {img ? (
            <img src={img} alt={ad.title} className="w-full h-full object-cover" onError={(e)=>{e.currentTarget.src="/images/no-image.webp"}} />
          ) : (
        ...
        <div className="mt-2 text-center">
          <div className="text-white font-semibold text-sm truncate" title={ad.title}>{ad.title}</div>
          <div className="flex items-center justify-center gap-1 text-xs text-white/75 mt-0.5">
            <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8"><path d="M12 2C8 2 4 6 4 11c0 5.5 7 11 8 11s8-5.5 8-11c0-5-4-9-8-9z"/><circle cx="12" cy="11" r="3"/></svg>
            <span className="truncate">{ad.company}{ad.region ? ` · ${ad.region}` : ""}</span>
          </div>
        </div>
      </div>
    </Link>
